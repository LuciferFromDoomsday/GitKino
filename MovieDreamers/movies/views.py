    ...
    if request.method == "POST":
        ...
        if needed_city is not None:
            ...
            return render(request, 'home.html',
                              {'cinemas': needed_cinemas, 'cities': cities_list, 'sessions': needed_sessions,
                               'movies': needed_movies})
...
def detail(request, movie_id):
    ...
    needed_sessions= []
    needed_cities=[]
    sessions_list= Sessions.objects.filter(film_id=m.film_id)
    for session in sessions_list:



        cinema = Cinema.objects.filter(theatre_id= session.cinema_id)

        if cinema is not None:

            needed_sessions.append(session)
            for cinema1 in cinema:
                cinemas.append(cinema1)
    ...
    return render(request, 'movies/detail.html', {'movie': m, 'latest_comment_list': latest_comment_list , 'sessions' :needed_sessions,'cinemas':cinemas } )
...
def final_buy(request):
    if request.method == "POST":
        user = request.user
        time = request.POST.get('time')
        seats = request.POST.get('seats')
        price =request.POST.get('price')
        msg = "YOU have just bought a ticket ! My congratulations comrade! That you eveded all bugs of this  site and could buy a ticket! "
        email_sender(msg, user.email, user.username)
        return HttpResponse("Successfully bought ! Check up your gmail))")

    return render(request, "movies/place.html")


def buy_ticket(request, session_id):
    if request.method == "POST":

        need_sessions = Sessions.objects.filter(id=session_id)
        date = need_sessions[0].session_date
        time = need_sessions[0].session_time
        needed_films = Movie.objects.filter(film_id=need_sessions[0].film_id)
        needed_film_title = str(needed_films[0].movie_title)

        return render(request, "movies/place.html" , {'date' : date , 'time': time , 'film': needed_film_title})
    return HttpResponse("Bought failed")
